<template>
    <div class="bubble-controls bg-gray-50 rounded-lg p-4">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div class="space-y-2">
                ...
                >
                <select
                    v-model="localBubbleSize"
                    @change="handleBubbleSizeChange"
                    class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                >
                    <option value="">請選擇氣泡大小</option>
                    <optgroup
                        v-for="group in groupedOptions"
                        :key="group.type"
                        :label="group.label"
        ...
        <div class="mt-4 p-3 bg-blue-50 rounded-lg">
            <div class="flex items-start space-x-2">
                ...
                <div class="text-sm text-blue-700">
                    ...
                    <p>
                        選擇不同的數值指標作為 X 軸、Y
                        軸和氣泡大小，每個氣泡代表一家公司，氣泡大小反映所選指標的數值。
                    </p>
                </div>
            </div>
        </div>
...
const presets = [
    ...
    },
    {
        ...
        bubbleSize: "total_funding_usd",
    },
    {
        name: "排名分析",
        xAxis: "rank",
        yAxis: "total_funding_usd",
        bubbleSize: "founded_year",
    },
    {
        name: "投資輪次",
...
const handleBubbleSizeChange = () => {
    emit("update:bubbleSize", localBubbleSize.value);
    emit("update", localXAxis.value, localYAxis.value, localBubbleSize.value);
};
...
const applyPreset = (preset: {
    ...
    localBubbleSize.value = preset.bubbleSize;

    emit("update:xAxis", preset.xAxis);
    emit("update:yAxis", preset.yAxis);
    emit("update:bubbleSize", preset.bubbleSize);
    emit("update", preset.xAxis, preset.yAxis, preset.bubbleSize);
};
